...
public class PersonaServiceRS {
    ...
    @GET
    @Produces(value = MediaType.APPLICATION_JSON)
    @Path("{id}")//hace referencia al path: /personas/(id) ej. /personas/1   ........ es lo que estamos buscando
    public Persona encontrarPersona(@PathParam("id") int id){
        //buscamos a la persona en la base de datos 
            Persona persona = personaDao.encontrarPersona(new Persona(id));//busca a una persona por id 
            System.out.println("persona encontrada: " + persona);
            return persona;
    }
    
    @POST//recibe un objeto persona 
    @Consumes(value = MediaType.APPLICATION_JSON)//recibe a un objeto
    @Produces(value =   MediaType.APPLICATION_JSON)
